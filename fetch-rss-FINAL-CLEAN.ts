...
serve(async (req) => {
  ...
  try {
    ...
    // 处理每篇文章
    for (const rssArticle of rssArticles.slice(0, 5)) { // 限制处理5篇
      try {
        ...
        const { data: existingArticle } = await supabaseClient
          .from('articles')
          .select('id')
          .eq('original_article_link', rssArticle.link)
          .maybeSingle()
        ...
        const article = {
          ...
          content: fullContent,           // 全文内容（需要先在数据库添加此字段）
          summary_ai: aiAnalysis.summary, // AI摘要
          score_ai: aiAnalysis.score,     // AI评分
          is_selected: false
        }
...
// ===== 步骤1: RSS拉取 - 只获取基础字段 =====
async function fetchRSSBasicInfo(rssUrl: string) {
  console.log('📡 开始RSS拉取...')
...
// ===== 步骤2: Jina API 读取全文 =====
async function fetchFullTextWithJina(url: string): Promise<string> {
...
// ===== 步骤3: Gemini LLM 分析全文 - 摘要和打分 =====
async function analyzeWithGemini(title: string, fullContent: string, verticalName: string) {
